...
def write_dictionary_markdown(dictionary: dict, output_dir: str | Path) -> Path:
    ...
    lines = [
        f"# Data Dictionary: {ds.get('source_file')}",
        "",
        "## Dataset summary",
    ...
    for r in rows:
        lines.append(
            f"| {r['column_name']} | {r['display_name']} | {r['description'] or '(blank - review required)'} | {r['physical_type']} | {r['semantic_role']} | {r['semantic_role_confidence']} | {r['nullable']} | {r['review_required']} |"
        )
    ...
    for r in rows:
        if r["review_required"]:
            ...
            lines.append(f"- **{r['column_name']}**: {notes}")
    ...
    lines.extend(
        [
            ...
            "- Semantic roles are suggestions, not confirmed business definitions.",
            "- Possible sensitive fields are hints, not compliance classification.",
